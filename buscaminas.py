...
def tocar (mapa , tablero_oculto , coord):
    ...
    if tablero_oculto[coord] == -1:
        ...
        vecinos = vecinos_de(tablero_oculto , coord)
        while second_check:
            for i in range(len(vecinos)):
                mapa[vecinos[i]] = tablero_oculto[vecinos[i]]
                if tablero_oculto[vecinos[i]] == 0:
                    vecinos_cero.append(vecinos[i])
            for j in range(len(vecinos_cero)):
                colocar_ceros(mapa , tablero_oculto , vecinos_cero[j])   
...
def jugar_una_partida(filas, columnas, bombas):
    ...
    while not exploto and seguir_jugando(mapa , oculto):
        ...
        if en_rango(oculto , coord):
            if mapa[coord] != ".":
                print("Elija un casillero que no haya sido revelado")
            else:
...
x = jugar_una_partida(filas , columnas , bombas)
